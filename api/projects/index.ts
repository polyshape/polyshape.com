...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    try {
      const blobs = await getAll('projects');
      res.status(200).json(blobs);
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : String(e);
      res.status(500).json({ ok: false, error: message || 'Failed to list projects' });
    }
    return;
  }
  ...
  if (req.method === 'POST') {
    try {
      ...
      const result = await upload('projects', filename, jsonData);

      res.status(201).json({ ok: true, blob: result });
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : String(e);
      res.status(500).json({ ok: false, error: message || 'Failed to create project' });
    }
    return;
  }
